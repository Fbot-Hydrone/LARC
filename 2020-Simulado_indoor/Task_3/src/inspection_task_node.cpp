        ...
        if (flag)
        {
            ...
            if (main_image.foundMarkRect())
            {
                ...
                if (base.foundRect())
                {
                    ...
                    // string txt = to_string(detectPercent.isON[0]) + " " + to_string(detectPercent.isON[1]) + " " + to_string(detectPercent.isON[2]) + " " + to_string(detectPercent.isON[3]);
                    
                    // putText(sensorGrayscale, txt, Point(20, 100), FONT_HERSHEY_DUPLEX, 1.5, 140, 2);

                    // std::stringstream stream;
                    // stream << std::fixed << std::setprecision(2) << detectPercent.ratioCenter;
                    // stream << " ";
                    // stream << std::fixed << std::setprecision(2) << detectPercent.ratioTopBall;
                    // stream << " ";
                    // stream << std::fixed << std::setprecision(2) << detectPercent.ratioBotBall;
                    // stream << " ";
                    // stream << std::fixed << std::setprecision(2) << detectPercent.ratioNonZeroImg;

                    // string text = stream.str();

                    // putText(sensorGrayscale, text, Point(20, 50), FONT_HERSHEY_DUPLEX, 1.0, 140, 2);
