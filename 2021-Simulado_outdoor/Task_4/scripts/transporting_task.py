...
FIXED_BASE_2 = Point(-19.15, -21, 13.6)  # Base mais alta
FIXED_BASE_3 = Point(-54.15, -35.1, 9.85)  # Base perto do cano
MOVING_BASE_1 = Point(65.0, 0.0, 1.7)      # Movel atras do pier
MOVING_BASE_2 = Point(40, -55, 1.7)        # Movel aberta para mar
MOVING_BASE_3 = Point(-10.0, 30.0, 1.7)    # Base perto da costal
...
def findBase():
    ...
    while True:
        ...
        if (distancePixels.data[1] + 30) > PX_THRESHOLD:
            delta_x = ADJUSTED_STEP
        elif (distancePixels.data[1] + 30) < -PX_THRESHOLD:
            delta_x = -ADJUSTED_STEP
        else:
            delta_x = 0.0
        ...
        if (abs(distancePixels.data[0]) < PY_THRESHOLD) and (abs(distancePixels.data[1] + 30) < PX_THRESHOLD):
            print("Going down...")
...
def mission_planner():
    ...
    uav.moveTo(MOVING_BASE_1.x, MOVING_BASE_1.y, 4.8, 4.8)
    ...
    uav.updateGlobalPosition()
    print("Moving z axis. global: {:.2f}. Moving to {:.2f}".format(uav.globalPosition.z, 10 - (uav.globalPosition.z + 1.7)))
    ...
    try:
        found = rospy.wait_for_message('/block_detector/found_qrcode', Bool, timeout=14)
    except(rospy.ROSException) as e:
        print('\nQRCODE nao encontrado\n')
