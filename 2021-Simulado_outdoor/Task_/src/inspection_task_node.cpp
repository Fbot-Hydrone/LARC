...
ros::Publisher in_range; 
ros::Publisher detected_sensor; 
ros::Publisher sensor_values;

std_msgs::Bool detectedSensorMsg;
...
int ARR_MAXMARCADOR_HSV[3] = {45, 25, 95};
...
public:
    ...
    Rect rectBotBall, rectTopBall;
    int botBallArea, topBallArea;

    Percent()
    {
        this->rectBotBall = Rect(Point(16, 20), Point(26, 28));
        this->rectTopBall = Rect(Point(2, 3), Point(14, 11));

        this->botBallArea = this->rectBotBall.width * this->rectBotBall.height;
        this->topBallArea = this->rectTopBall.width * this->rectTopBall.height;
    ...
    void predictCenter()
    {
        ...
        this->isON[0] = (( this->ratioCenter - 0.15 ) <= 0);
    }


    void predictTopBall()
    {
        Mat imgTopBall = this->imgMain(this->rectTopBall);
        ...
        this->ratioTopBall = nonZeroTopBall / float(this->topBallArea);
    ...
    void predictBotBall()
    {
        ...
        this->ratioBotBall = nonZeroBotBall / float(this->botBallArea);
...
public:
    ...
    Mat img_lab_can1_C1, img_hsv_can3_C1, imgMark;
    Mat morph_kernel;
    ...
    bool foundMark()
    {
        ...
        if (succ) {
            ...
            float angle = this->markRotatedRect.angle;
            Size size = this->markRotatedRect.size;

            if (this->markRotatedRect.angle < -45.) {
                angle += 90.0;
                swap(size.width, size.height);
            }

            M = getRotationMatrix2D(this->markRotatedRect.center, angle, 1.0);

            warpAffine(this->image, rotated, M, this->image.size(), INTER_CUBIC);

            getRectSubPix(rotated, size, this->markRotatedRect.center, cropped);

            this->imgMark = cropped.clone();
...
public: 
    ...
    bool found()
    {
        ...
        vector<vector<Point>> contours;
        findContours(imageGrayscale, contours, RETR_LIST, CHAIN_APPROX_SIMPLE);
        ...
        if (biggest_area != 0 && razao(this->biggest_rect) <= 1.5) {
            Mat M, rotated, cropped;
            float angle = this->biggest_rect.angle;
            Size size = this->biggest_rect.size;

            if (this->biggest_rect.angle < -45.) {
                angle += 90.0;
                swap(size.width, size.height);
            }

            M = getRotationMatrix2D(this->biggest_rect.center, angle, 1.0);

            warpAffine(this->image, rotated, M, this->image.size(), INTER_CUBIC);

            getRectSubPix(rotated, size, this->biggest_rect.center, cropped);

            this->sensor = cropped.clone();
            cv::resize(this->sensor, this->sensor, Size(400, 400));
...
int contImageCallbacks = 0;
int contNotFoundSensor = 0;
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
    try
	{
        if (flag)
        {
            ...
            bool checkFoundBase;
            if (isMovingBase)
                checkFoundBase = main_image.foundMark();
            else
                checkFoundBase = main_image.foundMarkRect();

            if (checkFoundBase)
            {
                if (isMovingBase)
                    base.set(main_image.imgMark);
                else
                    base.set(main_image.image, main_image.mark);

                // Crop da base encontrada
                // base.show("BASE");

                if (base.found() && !base.sensor.empty())
                {
                    isMovingBase = false;

                    detectedSensorMsg.data = true;
                    detected_sensor.publish(detectedSensorMsg);

                    // imshow("sensor", base.sensor);
                ...
                else
                {
                    ...
                    if (contNotFoundSensor >= 8){
                        detectedSensorMsg.data = false;
                        detected_sensor.publish(detectedSensorMsg);
                        contNotFoundSensor = 0;
                    }
                }
            ...
            if (contImageCallbacks >= MAX_ITERATIONS)
            {
                contImageCallbacks = 0;

                destroyAllWindows();

                topValue = getIndexOfMaxElement(vecTopNumber);
                botValue = getIndexOfMaxElement(vecBotNumber);
                ...
                std_msgs::ByteMultiArray arrayIsRight;
                std_msgs::Int16MultiArray arrayValues;

                arrayIsRight.data.clear();
                arrayValues.data.clear();


                // printf(ANSI_COLOR_GREEN "Valor de gas detectado: %d" ANSI_COLOR_RESET "\n", topValue)
                arrayValues.data.push_back(topValue);
                if (gasIsInRange(topValue)){
                    arrayIsRight.data.push_back(1);
                }else{
                    arrayIsRight.data.push_back(0);
                }


                arrayValues.data.push_back(botValue);
                if (ajusteZeroIsInRange(botValue)){
                    arrayIsRight.data.push_back(1);
                }else{
                    arrayIsRight.data.push_back(0);
                }

                in_range.publish(arrayIsRight);     // [0] => gas, [1] => ajuste de zero
                sensor_values.publish(arrayValues); // [0] => valor gas [1] => valor zero

                // RESETA OS VALORES PARA O PROXIMO MARCADOR
                fillImageBlack(sensorGrayscale);
...
int main(int argc, char **argv)
{
    ...
    // in_range = n.advertise<std_msgs::Bool>("/gas_detector/in_range", 1);
    in_range = n.advertise<std_msgs::ByteMultiArray>("/gas_detector/in_range", 1);
    detected_sensor = n.advertise<std_msgs::Bool>("/gas_detector/detected", 1);
    sensor_values = n.advertise<std_msgs::Int16MultiArray>("/gas_detector/values", 1);
