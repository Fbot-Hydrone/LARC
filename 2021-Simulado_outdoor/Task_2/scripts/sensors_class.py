...
class Sensors:
	...
	def processImage(self):
		...
		self.bad_mark = cv2.morphologyEx(self.bad_mark, cv2.MORPH_CLOSE, kernel, iterations = 2)
		self.good_mark = cv2.morphologyEx(self.good_mark, cv2.MORPH_CLOSE, kernel, iterations = 2)
	...
	def getRefFrame(self, final_img, tag):
		...
		contours, hier = cv2.findContours(final_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
